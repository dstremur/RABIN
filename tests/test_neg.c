...
// Random signed mpz in [0, 2^bits) with a random sign; returns its decimal
// string (caller frees).
...
static void run_edge_cases()
{
  ...
  run_case("one", "1");
  run_case("neg-one", "-1");
  run_case("u64-max", "18446744073709551615");
  run_case("neg-u64-max", "-18446744073709551615");
  run_case("2^64", "18446744073709551616");
  run_case("neg-2^64", "-18446744073709551616");
...
int main()
{
  ...
  print_table_header();
  benchmark_neg(4096, bench_budget(4096), state);
  benchmark_neg(65536, bench_budget(65536), state);
  benchmark_neg(1000000, bench_budget(1000000), state);
  print_table_footer();

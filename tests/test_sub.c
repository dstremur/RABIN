 ...
 * test_sub.c
 *
 * Unified GMP-verified test suite for bn_sub.
 *
 *   1. Edge cases: 0, 1, -1, 2^64-1, 2^64 boundaries, borrow cascades +
 *      pointer aliasing (r == a and r == b)
 *   2. 1000 randomized cases (1..4096 bits) vs mpz_sub
...
static void run_case(const char* name, const char* a_str, const char* b_str)
{
  ...
  snprintf(detail, sizeof(detail), "bn_sub [%s] a=%s b=%s", name, a_str, b_str);
  ...
  bn_init_val(&a, a_str);
  bn_sub(&a, &a, &b);
  snprintf(detail, sizeof(detail), "bn_sub [%s, r==a] a=%s b=%s", name, a_str,
           b_str);
  ...
  bn_init_val(&b, b_str);
  bn_sub(&b, &a, &b);
  snprintf(detail, sizeof(detail), "bn_sub [%s, r==b] a=%s b=%s", name, a_str,
           b_str);
...
static void run_edge_cases()
{
  ...
  run_case("zeros", "0", "0");
  run_case("one - one (to +0)", "1", "1");
  run_case("zero - one", "0", "1");
  run_case("one - zero", "1", "0");
  run_case("neg-one - neg-one", "-1", "-1");
  run_case("u64-max - u64-max (to +0)", "18446744073709551615",
           "18446744073709551615");
  run_case("1 - u64-max (cross-limb borrow)", "1", "18446744073709551615");
  run_case("2^64 - 1 (exact limb borrow)", "18446744073709551616", "1");
  run_case("2^64 - u64-max", "18446744073709551616", "18446744073709551615");
  run_case("u64-max - 2^64 (negative result)", "18446744073709551615",
           "18446744073709551616");
  run_case("neg - pos (add magnitudes)", "-18446744073709551615", "1");
  run_case("pos - neg (add magnitudes)", "1", "-18446744073709551615");
  run_case("huge - small", "123456789012345678901234567890", "2");
...
{
  printf("\n--- bn_sub: %d randomized cases vs mpz_sub ---\n", FUZZ_ITERATIONS);
...
static void benchmark_sub(int bits, double target_sec, gmp_randstate_t state)
{
  ...
  assert_match("bn_sub (benchmark)", "(bench input)", "(bench input)", &bn_res,
               mpz_res);

  char size_info[32];
  snprintf(size_info, sizeof(size_info), "%d bits", bits);
  print_table_row("bn_sub", size_info, total_custom / ops_custom,
                  total_gmp / ops_gmp);
...
int main()
{
  ...
  print_table_header();
  benchmark_sub(4096, bench_budget(4096), state);
  benchmark_sub(65536, bench_budget(65536), state);
  print_table_footer();

 ...
 *   2. 1000 randomized cases (1..4096 bits) vs mpz_sqrt
...
static void run_edge_cases()
{
  ...
  run_case("zero", "0");
  run_case("one", "1");
  run_case("two (floor)", "2");
  run_case("three (floor)", "3");
  run_case("u64-max (floor of sqrt)", "18446744073709551615");
  run_case("2^64", "18446744073709551616");

  // perfect squares: (2^k)^2 = 2^(2k)
  mpz_t sq;
  mpz_init(sq);
  int exps[] = {32, 64, 128, 256, 512, 1024, 2048};
  for (size_t i = 0; i < sizeof(exps) / sizeof(exps[0]); i++) {
    mpz_set_ui(sq, 2);
    mpz_pow_ui(sq, sq, (unsigned long)(2 * exps[i]));
    char* s = mpz_get_str(NULL, 10, sq);
    char name[64];
    snprintf(name, sizeof(name), "perfect square 2^(%d)", 2 * exps[i]);
    run_case(name, s);
    free(s);
  }
  mpz_clear(sq);
...
int main()
{
  ...
  print_table_header();
  benchmark_isqrt(2048, bench_budget(2048), state);
  benchmark_isqrt(8192, bench_budget(8192), state);
  benchmark_isqrt(32768, bench_budget(32768), state);
  benchmark_isqrt(65536, bench_budget(65536), state);
  print_table_footer();
